...
<body>
	...
	<?php
		...
		$query = $db->query("SELECT * FROM following WHERE uid='$user' ");
		...
		while($row = $query->fetch_object())
		{
			...
			$query2 = $db->query("SELECT * FROM following WHERE uid='$curruser' AND followingid='$following'");
			...
			echo '<div style="width: 33%; text-align: center; ">';
				...
				$query3 = $db->query("SELECT * FROM profilepics WHERE uid = '$following' ");
			...
			$query1 = $db->query("SELECT username FROM users WHERE userid = '$following' ");
			$row1 = $query1->fetch_object();
			echo '<div style="width: 33%; padding: 20px; text-align: center; ">';
			?>

			<a href="userpage.php?uid=<?php echo $following ?>">
			...
			echo '<div style="width: 33%; text-align: center; ">';
			if($following == $curruser)
			{
				//display nothing for the present user.
			}
			
			else if($flag==0){ 
				$str = "follow.php?userid=".$curruser."&followid=".$following;
			?>
			...
			<?php
					...
					$str = "unfollow.php?userid=".$curruser."&followid=".$following;
			?>
